...
\Large \only<1>{La función de costo epistémico\phantom{evolutiva}}\only<2->{La función de costo evolutiva\phantom{epistémico}}
...
\only<9->{
\begin{textblock}{160}(0,40) \centering \huge
\begin{equation*}
...
\only<10>{
\begin{textblock}{160}(0,72) \centering \Large
Hay más propiedades que emergen naturalmente de los \\ procesos multiplicativos de evaluación de las variantes
\end{textblock}
}
...
\includegraphics[width=0.75\textwidth]{figuras/kelly}
\end{textblock}
...
\begin{frame}[plain]
\begin{textblock}{160}(0,4)
\centering \LARGE Bibliografía recomendada
\end{textblock}
\vspace{1.2cm}


\only<2->{
\begin{textblock}{150}(5,14)
1. La reformulación ergódica de la teoría de Teoría de Utilidad Esperada. \\[0.2cm]

\normalsize
\ \ \ $\bullet$ Peters, O. \textit{The ergodicity problem in economics} Nature. 2019.

\ \ \  $\bullet$ Peters, O; Amdamou, A. \textit{The time interpretation of expected utility theory} Arxiv. 2021.
\end{textblock}
}


\only<3->{
\begin{textblock}{150}(5,35)
2. Isomorfismo probabilidad-evolución \\[0.2cm]

\normalsize
\ \ \ $\bullet$ Cz\'egel, D; Zachar, I; Szathm\'ary, E. \textit{Multilevel selection as Bayesian inference, major transitions in individuality as structure learning} Royal Society open science. 2019.

\ \ \  $\bullet$ Cz\'egel, D; Giaffar, H; Tenenbaum, JB; Szathm\'ary, E. \textit{Bayes and Darwin: How replicator populations implement Bayesian computations.} BioEssays. 2022.
\end{textblock}
}

\only<4->{
\begin{textblock}{150}(5,66)
3. Toma de decisiones como un problema de inferencia (evolutivo) \\[0.2cm]

\normalsize
\ \ \ $\bullet$ Levine, S. \textit{Reinforcement Learning and Control as Probabilistic
Inference: Tutorial and Review} Arxiv. 2018.

\ \ \  $\bullet$ Parr, T; Pezzulo, G; Friston, K. \textit{Active Inference} MIT Press. 2022.
\end{textblock}
}
